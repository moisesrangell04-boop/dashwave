...
async function ensureTenant() {
  ...
  return prisma.tenant.create({
    data: { id: TENANT_ID, name: 'Wave CRM', slug: 'wave-crm' },
  });
}

async function ensureWorkspace() {
  const existing = await prisma.workspace.findUnique({ where: { id: WORKSPACE_ID } });
  if (existing) return existing;
  return prisma.workspace.create({
    data: { id: WORKSPACE_ID, tenantId: TENANT_ID, name: 'Principal' },
  });
}
